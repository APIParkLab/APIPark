...
import {
  filterServiceList,
  initialServiceHubListState,
  SERVICE_HUB_LIST_ACTIONS,
  ServiceHubListActionType
} from './ServiceHubList.tsx'
...
export const ServiceHubGroup = ({ children, filterOption, dispatch }: ServiceHubGroup) => {
  ...
  const onSearchWordChange = (e: string) => {
    ...
    dispatch({
      ...
      payload: filterServiceList({ ...filterOption, keyword: e })
    })
    ...
    fetchData<BasicResponse<{ catalogues: CategorizesType[]; tags: EntityItem[] }>>('catalogues', {
      ...
      if (code === STATUS_CODE.SUCCESS) {
        ...
        dispatch({
          ...
          payload: [...data.tags.map((x: EntityItem) => x.id), 'empty']
        })
      } else {
      ...
      data?.map((item) => {
        if ((item as CategorizesType).children) {
          return {
            title: item.name,
            key: item.id,
            children: loop((item as CategorizesType).children)
          }
        }
        return {
          title: item.name,
  ...
  const onCheckHandler =
    (type: 'SET_SELECTED_CATE' | 'SET_SELECTED_TAG' | 'SET_SELECTED_PARTITION') => (checkedKeys: string[]) => {
      dispatch({ type: SERVICE_HUB_LIST_ACTIONS[type], payload: checkedKeys })
      dispatch({ type: SERVICE_HUB_LIST_ACTIONS.LIST_LOADING, payload: true })

      dispatch({
        type: SERVICE_HUB_LIST_ACTIONS.SET_SERVICES,
        payload: filterServiceList({
          ...filterOption,
          [(type === 'SET_SELECTED_CATE'
            ? 'selectedCate'
            : type === 'SET_SELECTED_TAG'
              ? 'selectedTag'
              : 'selectedPartition') as keyof typeof filterOption]: checkedKeys
        })
      })
      dispatch({ type: SERVICE_HUB_LIST_ACTIONS.LIST_LOADING, payload: false })
    }
      ...
      <div className="w-[220px] border-0 border-solid border-r-[1px] border-r-BORDER">
        <div className=" h-full">
          <Input
            ...
            onChange={(e) => debounce(onSearchWordChange, 500)(e.target.value)}
            allowClear
            placeholder={$t('搜索服务')}
