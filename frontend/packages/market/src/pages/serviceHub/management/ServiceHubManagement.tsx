...
import { App, Avatar, Card, Empty, Menu, MenuProps, Radio, Tooltip } from 'antd'
...
export default function ServiceHubManagement() {
  ...
  const { getTeamAccessData, cleanTeamAccessData, checkPermission, getGlobalAccessData, accessInit, state } =
    useGlobalContext()
  ...
  const getServiceList = (dataType?: 'block' | 'list') => {
    ...
    return fetchData<BasicResponse<{ apps: ServiceHubAppListItem }>>(
      !checkPermission('system.workspace.application.view_all') ? 'my_apps' : 'apps',
      {
        method: 'GET',
        eoParams: { team: dataType === 'list' ? undefined : teamId, keyword: tableSearchWord },
        eoTransformKeys: ['api_num', 'subscribe_num', 'subscribe_verify_num', 'auth_num', 'create_time', 'can_delete']
      }
    )
      .then((response) => {
  ...
  const openModal = async (type: 'add' | 'edit' | 'delete') => {
    ...
    modal.confirm({
      ...
      onOk: () => {
        switch (type) {
          case 'add':
            return addManagementRef.current?.save().then((res) => {
              if (res === true) {
                getTeamsList()
  ...
  const dataShowTypeOptions = useMemo(
    () => DATA_SHOW_TYPE_OPTIONS.map((x) => ({ label: $t(x.label), value: x.value })),
    [state.language]
  )
          ...
          itemContent={(index) => {
            ...
            return (
              <div className="pt-[20px]">
                {item.type === 'addNewItem' ? (
                  <WithPermission access="team.application.application.add" showDisabled={false}>
                    <Card
                      className="shadow-[0_5px_10px_0_rgba(0,0,0,0.05)] rounded-[10px] overflow-visible cursor-pointer h-[180px]  transition duration-500 hover:shadow-[0_5px_20px_0_rgba(0,0,0,0.15)] hover:scale-[1.05]"
                      classNames={{
                        body: 'h-[180px] flex items-center justify-center cursor-pointer'
                      }}
                      onClick={() => {
                        openModal('add')
                      }}
                    >
                      <div className="flex items-center">
                        <Icon icon="ic:baseline-add" width="18" height="18" />
                        <span>{$t('添加消费者')}</span>
                      </div>
                    </Card>
                  </WithPermission>
                  ...
                  <Card
                    ...
                    onClick={() => {
                      ...
                      navigateTo(`/consumer/${teamId}/inside/${item.id}/service`)
                    }}
                  >
                    <span className="break-all line-clamp-3">{item.description || $t('暂无消费者描述')}</span>
                  </Card>
                )}
              </div>
            )
    ...
    <>
      {teamList && teamList.length > 0 ? (
        <InsidePage
          ...
          pageTitle={$t('消费者')}
          description={$t(
            '创建并管理自己的消费者实体，每个消费者可以订阅多个API服务，确保在调用之前已获得相应权限。你可以为消费者生成 API 密钥等鉴权方式，用于安全地调用 API 服务'
          )}
          showBorder={false}
          scrollPage={false}
          ...
          customBtn={
            <Radio.Group
              ...
              onChange={(e) => {
                setDataShowType(e.target.value)
                setTableHttpReload(true)
        ...
        >
          {dataShowType === 'block' ? (
            ...
            <TableArea
              ...
              getServiceList={() => getServiceList('list')}
              ref={tableRef}
              addNewApp={() => openModal('add')}
              setTableHttpReload={setTableHttpReload}
  ...
  return (
    <div className="flex">
      <Avatar
        shape="square"
        size={50}
        className=" bg-theme rounded-[12px]"
        icon={<iconpark-icon className="" name="auto-generate-api"></iconpark-icon>}
      />
      <div className="pl-[20px] w-[calc(100%-50px)]">
        ...
        <div className="mt-[10px] h-[20px] flex items-center font-normal">
          <Tooltip
            title={$t('订阅的服务数量：已通过 (0) 个，申请中 (1) 个', [
              service.subscribeNum ?? '-',
              service.subscribeVerifyNum ?? '-'
            ])}
          >
...
const TableArea = memo(
  forwardRef((props: TableAreaProps, ref: Ref<TableAreaHandle>) => {
