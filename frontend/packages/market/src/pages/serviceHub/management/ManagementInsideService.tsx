...
import { App, Button, Card, Dropdown, Empty, Input, Tag } from 'antd'
...
export default function ManagementInsideService() {
  ...
  const cancelSubscribeApply = (entity: TenantManagementServiceListItem) => {
    return new Promise((resolve, reject) => {
      fetchData<BasicResponse<null>>('application/subscription/cancel_apply', {
        method: 'POST',
        eoParams: { subscription: entity.id!, application: appId!, team: teamId }
      })
        .then((response) => {
  ...
  const cancelSubscribe = (entity: TenantManagementServiceListItem) => {
    return new Promise((resolve, reject) => {
      fetchData<BasicResponse<null>>('application/subscription/cancel', {
        method: 'POST',
        eoParams: { subscription: entity.id!, application: appId!, team: teamId }
      })
        .then((response) => {
  ...
  const openModal = async (type: 'view' | 'cancelSub' | 'cancelSubApply', entity?: TenantManagementServiceListItem) => {
    ...
    switch (type) {
      case 'view': {
        ...
        const { code, data, msg } = await fetchData<BasicResponse<{ approval: SubscribeApprovalInfoType }>>(
          'app/subscription/approval',
          {
            method: 'GET',
            eoParams: { subscription: entity!.id, app: appId, team: teamId },
            eoTransformKeys: ['apply_project', 'apply_team', 'apply_time', 'approval_time']
          }
        )
        message.destroy()
        if (code === STATUS_CODE.SUCCESS) {
      ...
      onOk: () => {
        switch (type) {
          case 'view':
            return true
          case 'cancelSubApply':
            return cancelSubscribeApply(entity!).then((res) => {
              if (res) {
                getServiceList()
                refreshGroup?.()
              }
          ...
          case 'cancelSub':
            return cancelSubscribe(entity!).then((res) => {
              if (res) {
                getServiceList()
                refreshGroup?.()
              }
      ...
      ? {
          ...
          label: (
            ...
            <Button
              ...
              onClick={() => openModal('cancelSubApply', entity)}
            >
              {$t('取消订阅申请')}
            </Button>
            // </WithPermission>
      ...
      : {
          ...
          label: (
            ...
            <Button
              ...
              type="text"
              className="h-[32px] border-none p-0 flex items-center bg-transparent "
              onClick={() => openModal('cancelSub', entity)}
            >
              {$t('取消订阅')}
            </Button>
            //  </WithPermission>
  ...
  const getServiceList = () => {
    fetchData<BasicResponse<{ subscriptions: TenantManagementServiceListItem }>>('application/subscriptions', {
      method: 'GET',
      eoParams: { application: appId, team: teamId },
      eoTransformKeys: ['apply_status']
    }).then((response) => {
  ...
  return (
    <div className=" mx-auto h-full pt-[32px]">
      ...
      {(keyword ? serviceList.filter((x) => x.service.name.includes(keyword)) : serviceList)?.length > 0 ? (
              ...
              >
                <div className="flex justify-between items-center w-full">
                  <span>
                    ...
                    {item.applyStatus === 1 && (
                      <Tag className="ml-[8px]" bordered={false} color="orange">
                        {$t('审核中')}
                      </Tag>
                    )}
                  </span>
        ...
        <div className="bg-[#fafafa] overflow-hidden rounded-[10px]">
          <Empty className="m-[50px] " image={Empty.PRESENTED_IMAGE_SIMPLE} />
        </div>
      )}
    </div>
  )
