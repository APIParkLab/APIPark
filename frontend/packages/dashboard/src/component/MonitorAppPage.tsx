...
export default function MonitorAppPage(props: MonitorAppPageProps) {
  ...
  const getAppList = () => {
    return fetchData<{ apps: EntityItem[] }>('simple/apps/mine', { method: 'GET' })
      .then((resp) => {
    ...
    fetchTableData(data).then((resp) => {
      ...
      if (code === STATUS_CODE.SUCCESS) {
        exportExcel(
          ...
          APPLICATION_TABLE_GLOBAL_COLUMNS_CONFIG,
          data.statistics
        )
      } else {
        message.error(msg || $t(RESPONSE_TIPS.dataError))
  ...
  const getTablesData = (body: SearchBody) => {
    return fetchTableData(body)
      .then((resp) => {
        ...
        if (code === STATUS_CODE.SUCCESS) {
          return {
            data: data.statistics?.map((x: MonitorSubscriberData) => {
  ...
  return (
    <div className="h-full overflow-hidden">
        ...
        <div className="flex flex-wrap items-center row-gap-[12px]  pt-btnybase mr-btnybase">
          <div>
            ...
            <Select
              ...
              options={listOfApps}
              onChange={(value) => {
                setQueryData((prevData) => ({ ...(prevData || {}), apps: value }))
              }}
            />
          </div>
          <div>
      ...
      <div className="h-[calc(100%-100px)]">
        <MonitorTable
          ref={monitorAppTableRef}
          noTop={true}
          type="subscribers"
          id="dashboard_application"
          onRowClick={(record) => {
