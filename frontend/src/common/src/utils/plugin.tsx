...
// Removed virtual:__federation__ import
...
export async function loadRemoteModule(remoteModuleConfig: RemoteModuleConfig) {
  // @ts-ignore
  if (typeof window !== 'undefined' && window.__federation_method_setRemote) {
    // @ts-ignore
    window.__federation_method_setRemote(remoteModuleConfig.remoteName, {
      url: () => Promise.resolve(remoteModuleConfig.remoteEntry),
      format: 'esm',
      from: 'vite'
    })
    // @ts-ignore
    return await window.__federation_method_getRemote(remoteModuleConfig.remoteName, `./${remoteModuleConfig.exposedModule}`)
  }
  console.warn('Vite federation is not supported in Next.js build. Mocking loadRemoteModule.');
  return null;
}
